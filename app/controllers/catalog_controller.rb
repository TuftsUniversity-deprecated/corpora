...
class CatalogController < ApplicationController  
  ...
  def add_dca_admin_displays_awareness(solr_parameters, user_parameters)
    solr_parameters[:fq] ||= []
    solr_parameters[:fq] << "displays_tesim:corpora"
  end
...
#  CatalogController.solr_search_params_logic += [:add_access_controls_to_solr_params]
  ...
  configure_blacklight do |config|
    ...
    config.add_facet_field solr_name('places', :facetable), :label => 'Places', :limit => 7
    config.add_facet_field solr_name('concepts', :facetable), :label => 'Concepts', :limit => 7
    ...
    config.add_facet_field solr_name('corpora_collection', :facetable), :label => 'Collection', :limit => 7
  ...
  # respond to ajax request for internal and external references
